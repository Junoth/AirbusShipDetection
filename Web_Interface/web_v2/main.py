...
app.config['MONGO_URI'] = 'mongodb://localhost:27017/shipdb'
mongo = PyMongo(app)
...
def load_user(id):
    ...
    if u is None:
        return None
    return User(u['Email'], u['Username'], id)
...
def login():
    ...
    if request.method == 'POST':
        ...
        if mongo.db.posts.find_one({'Email':email}) is None:
            flask.flash('Email not exist or password is wrong,please try again', 'info')
        elif check_password_hash(mongo.db.posts.find_one({'Email':email})['Password'] ,passwd) == False:
            flask.flash('Email not exist or password is wrong,please try again', 'info')
        else:   
            id = mongo.db.posts.find_one({'Email':email})['Unique ID']
            name = mongo.db.posts.find_one({'Email':email})['Username'] 
            user = User(form.email, name, id) 
            login_user(user)
            next = flask.request.args.get('next')
            return flask.redirect(next or flask.url_for('intro2'))
    return render_template("login.html")
...
def register():
    ...
    if request.method == 'POST' and form.validate():
        if mongo.db.posts.find_one({'Email':form.email.data}) is None:
           ...
           flask.flash("This email has been used")
